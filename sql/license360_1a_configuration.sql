...
HOS &&cmd_getcpu_model. | head -10 > cpuinfo.txt
GET cpuinfo.sql
A ]' processor_model FROM DUAL;
0 SELECT q'[
/
SELECT REPLACE(REPLACE(REPLACE(REPLACE(q'[&&processor_model.]', CHR(9)), CHR(10)), ':'), 'model name ') processor_model FROM DUAL;
HOS rm cpuinfo.sql
...
@@&&fc_zip_driver_files. cpuinfo.txt
...
&&skip_ver_le_11_1. CASE WHEN q'{&&processor_model.}' LIKE '%5675%' THEN 'X2-2 ' END|| 
&&skip_ver_le_11_1. CASE WHEN q'{&&processor_model.}' LIKE '%2690%' THEN 'X3-2 ' END|| 
&&skip_ver_le_11_1. CASE WHEN q'{&&processor_model.}' LIKE '%2697%' THEN 'X4-2 ' END|| 
&&skip_ver_le_11_1. CASE WHEN q'{&&processor_model.}' LIKE '%2699%' THEN 'X5-2 ' END|| 
&&skip_ver_le_11_1. CASE WHEN q'{&&processor_model.}' LIKE '%8870%' THEN 'X3-8 ' END|| 
&&skip_ver_le_11_1. CASE WHEN q'{&&processor_model.}' LIKE '%8895%' THEN 'X4-8 or X5-8 ' END|| 
&&skip_ver_le_11_1. 'with '||cell.cnt||' storage servers' 
&&skip_ver_le_11_1. ELSE 'Unknown' END FROM cell
&&skip_ver_le_11_1.  UNION ALL
SELECT 'Processor:', q'{&&processor_model.}'
FROM   DUAL
UNION ALL
...
END;        
/
@@&&skip_lic_param_run_full.&&9a_pre_one.
...
END;
/
@@&&skip_ver_le_11.&&skip_lic_param_run_full.&&9a_pre_one.
...
END;        
/
@@&&skip_diagnostics.&&skip_lic_param_run_full.&&9a_pre_one.
...
@@&&fc_gen_select_star_query. '&&main_table.' 'sql_text'
@@&&skip_apex.&&skip_lic_param_run_full.&&9a_pre_one.
...
END;
/
@@&&skip_when_notsys.&&skip_lic_param_run_full.&&9a_pre_one.
...
END;
/
@@&&skip_ver_le_10.&&skip_ver_le_11.&&skip_when_notsys.&&skip_lic_param_run_full.&&9a_pre_one.
...
END;
/
@@&&skip_sqltxplain.&&skip_lic_param_run_full.&&9a_pre_one.
...
DEF title = 'CPU Usage Statistics';
@@&&fc_main_table_name. '&&is_cdb.' 'CDB_CPU_USAGE_STATISTICS' 'DBA_CPU_USAGE_STATISTICS'
BEGIN
  :sql_text := q'[
SELECT /*+ &&top_level_hints. */
       *
