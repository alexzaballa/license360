...
DEF title = 'DBMS_FEATURE_USAGE_REPORT - HTML';
DEF main_table = '';
BEGIN
  :sql_text := q'[
...
DEF title = 'DBMS_FEATURE_USAGE_REPORT - Text';
--DEF main_table = '';
BEGIN
  :sql_text := q'[
...
select
    ...
    DBID,  --custom
    (select name from v$database) dbname --custom
  from PFUS
  ...
  group by rollup(CON_ID), PRODUCT, DBID
